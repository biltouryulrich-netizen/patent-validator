...
app.post('/api/analyze', async (req, res) => {
  ...
  if (!apiKey) {
    return res.status(500).json({ error: 'Server is not configured. Please contact the administrator.' });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({
        error: err?.error?.message || `Analysis service error (${response.status})`
      });
    }
...
app.listen(PORT, () => {
  console.log(`Patent Validator running on port ${PORT}`);
});
